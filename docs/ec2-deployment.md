...
After deployment, the workflow verifies `http://127.0.0.1:<API_PORT>/v1/health` from inside EC2 and runs the API contract check from inside the deployed API container. It also reports whether `http://<EC2_HOST>:<API_PORT>/v1/health` is reachable from GitHub Actions; if that external check warns, the server deploy is healthy internally but the frontend Vercel rewrite still needs a reachable `SOUNDLOG_API_ORIGIN`. After the frontend deployment is rebuilt with the synced Vercel env, verify `https://soundlog.shop/api/soundlog/v1/health` and run the app repo's deployed-web check.
