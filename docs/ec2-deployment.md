...
| `PRODUCTION_ENV` | multiline `.env` | Server runtime environment except `DOCKER_IMAGE`. |
...
UPLOAD_PUBLIC_BASE_URL=http://52.79.185.121:4000
UPLOAD_PUBLIC_PATH=/uploads
USE_MOCK_DB=false
```

The workflow prepends `DOCKER_IMAGE=<dockerhub-username>/soundlog-server:<tag>` at deploy time, so do not include `DOCKER_IMAGE` in `PRODUCTION_ENV`.
...
gh secret set EC2_HOST --repo SoundLogTeam/SoundLogServer --body '52.79.185.121'
